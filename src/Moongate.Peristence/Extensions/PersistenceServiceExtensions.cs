...
public static class PersistenceServiceExtensions
{
    ...
    public static async Task SaveEntitiesAsync<T>(
        this IPersistenceManager service, string filePath, IEnumerable<T> entities,
    ...
    ) where T : class
    {
        if (!EntityTypeRegistry.IsRegistered<T>())
        {
            throw new InvalidOperationException(
                $"Entity type {typeof(T).Name} is not registered. Call EntityTypeRegistry.RegisterEntityType<{typeof(T).Name}>() first."
            );
        }
    ...
    public static async Task SaveMultipleEntitiesAsync(
        this IPersistenceManager service, string filePath, CancellationToken cancellationToken = default,
        params (Type Type, IEnumerable<object> Entities)[] entityCollections
    )
    {
        var entityDict = new Dictionary<Type, IList<object>>();
    ...
    public static async Task CreateBackupAsync(
        this IPersistenceManager service, string sourceFilePath, string backupFilePath,
        CancellationToken cancellationToken = default
    )
    {
    ...
    /// <param name="cancellationToken">Cancellation token</param>
    public static async Task CompactFileAsync(
        this IPersistenceManager service, string filePath, CancellationToken cancellationToken = default
    )
    {
